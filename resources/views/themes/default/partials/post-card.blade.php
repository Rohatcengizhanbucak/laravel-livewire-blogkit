...
@if ($translation)
    <article class="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        <div class="mb-3 flex flex-wrap items-center gap-3 text-xs font-medium uppercase tracking-wide text-neutral-500">
            @if ($post->category)
                <a href="{{ route('blog.categories.show', ['locale' => $currentLocale->code, 'slug' => $post->category->slug]) }}" class="text-neutral-700 hover:text-neutral-950">
                    {{ $post->category->name }}
                </a>
            @endif
        ...
        @if (filled($translation->excerpt))
            <p class="mt-3 text-base leading-7 text-neutral-600">{{ $translation->excerpt }}</p>
        @endif

        @if ($post->tags->isNotEmpty())
            <nav class="mt-5 flex flex-wrap gap-2" aria-label="{{ trans('blog.aria.post_tags', [], $currentLocale->code) }}">
                @foreach ($post->tags as $tag)
                    <a href="{{ route('blog.tags.show', ['locale' => $currentLocale->code, 'slug' => $tag->slug]) }}" class="rounded-full border border-neutral-200 px-3 py-1 text-xs font-medium text-neutral-600 hover:border-neutral-400 hover:text-neutral-950">
                        #{{ $tag->name }}
                    </a>
                @endforeach
            </nav>
