...
@if ($translation)
    <article class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div class="mb-3 flex flex-wrap items-center gap-3 text-xs font-medium uppercase tracking-wide text-slate-500">
            @if ($post->category)
                <a href="{{ route('blog.categories.show', ['locale' => $currentLocale->code, 'slug' => $post->category->slug]) }}" class="text-blue-700 hover:text-blue-900">
                    {{ $post->category->name }}
                </a>
            @endif
        ...
        <h2 class="text-2xl font-bold tracking-normal text-slate-950">
            <a href="{{ route('blog.show', ['locale' => $currentLocale->code, 'slug' => $translation->slug]) }}" class="hover:text-blue-700">
                {{ $translation->title }}
            </a>
        </h2>
