...
<html lang="{{ str_replace('_', '-', $currentLocale->code ?? app()->getLocale()) }}" dir="{{ $currentLocale->direction ?? 'ltr' }}">
    ...
    <body
        class="blog-public-surface min-h-screen bg-neutral-50 text-neutral-950 antialiased"
    ...
    >
        ...
        <header class="border-b border-neutral-200 bg-white/95">
            <nav class="mx-auto flex max-w-6xl flex-wrap items-center gap-x-6 gap-y-3 px-5 py-4" aria-label="{{ trans('blog.aria.primary_navigation', [], $currentLocale->code) }}">
                ...
                <div class="ml-auto flex flex-wrap items-center justify-end gap-4 text-sm">
                    ...
                    @if (($locales ?? collect())->count() > 1)
                        <div class="flex items-center rounded-full bg-neutral-100 p-1 text-xs font-semibold uppercase tracking-wide text-neutral-500" aria-label="{{ trans('blog.nav.language_switcher', [], $currentLocale->code) }}">
                            @foreach ($locales as $availableLocale)
                                ...
                                @if ($availableLocaleUrl)
                                    <a
                                        ...
                                        class="rounded-full px-2.5 py-1 transition {{ $isCurrentLocale ? 'bg-white text-neutral-950 shadow-sm' : 'hover:bg-white/80 hover:text-neutral-800' }}"
                                    >
                                ...
                                @else
                                    <span
                                        ...
                                        class="cursor-not-allowed rounded-full px-2.5 py-1 text-neutral-300"
                                    >
                                        {{ $availableLocale->code }}
                                    </span>
                                @endif
